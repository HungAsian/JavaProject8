// Note:  The copies of the the properties files in the NetBeans project 
// directory are spare copies.  The active copies are in the default package
// (src/classes) just above the package resourcebundledemo.
...
public class ResourceBundleDemo extends Application {
  ...
  public void initializeComboBox() {
    ...
    for (int i = 0; i < locales.length; i++) {
        String dn = locales[i].getDisplayName();
        if (dn.contains("(")) {
            cboLocale.getItems().add(dn);
            list.add(locales[i]);
        }
        locales = list.toArray(locales);
    }
  }
  // *****************************************************************
  // *****************************************************************
